...
import { mobOpenacc } from "./main.js";
import { openLoginForm } from "./main.js";
...
function displayProducts(data,filterd){
    ...
    data.forEach( product =>{
        ...
        productElement.querySelector('#heart').addEventListener('click',()=>{
            if(userData.n == undefined){
                callAlert("⚠️","Please login now",mobOpenacc,openLoginForm)
                
            }else{
                productElement.querySelector('#heart').style.color = 'red'
...
export function callAlert(alertName,msg,returnFntMobile,returnFntDesktop){
    $('#h-b-alert').css('display','flex');
    $('#h-b-alert .imoji').text(alertName)
    $('#h-b-alert p').text(msg)
    $('#h-b-alert button').on('click',()=>{
